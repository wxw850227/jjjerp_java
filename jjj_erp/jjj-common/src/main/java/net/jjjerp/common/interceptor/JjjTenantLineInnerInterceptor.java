package net.jjjerp.common.interceptor;
...
public class JjjTenantLineInnerInterceptor extends JsqlParserSupport implements InnerInterceptor {
            ...
            if(env.equals("uat")){
                ...
                if(requestDetail != null){
                    ...
                    if(sct == SqlCommandType.UPDATE || sct == SqlCommandType.DELETE){
                        if(requestDetail.getPath().startsWith("/api/admin/")){
                            // 如果是admin，并且是白名单可以操作
                            if (requestDetail.getUserName().equals("admin") && !requestDetail.getIp().equals(ip)) {
                                throw new BusinessException("演示环境，数据不允许删除");
                            }
                            if (requestDetail.getUserName().equals("test") && requestDetail.getPath().equals("/api/admin/passport/editPass")) {
                                throw new BusinessException("演示环境，test账户不允许修改密码");
                            }
                        }else if(requestDetail.getPath().startsWith("/api/saas/")){
    ...
    public boolean equals(final Object o) {
        if (o == this) {
            return true;
        } else if (!(o instanceof net.jjjerp.common.interceptor.JjjTenantLineInnerInterceptor)) {
            return false;
        } else {
            net.jjjerp.common.interceptor.JjjTenantLineInnerInterceptor other = (net.jjjerp.common.interceptor.JjjTenantLineInnerInterceptor)o;
    ...
    protected boolean canEqual(final Object other) {
        return other instanceof net.jjjerp.common.interceptor.JjjTenantLineInnerInterceptor;
    }
