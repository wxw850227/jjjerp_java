...
@Slf4j
@Api(value = "index", tags = {"其他出库单"})
@RestController
@RequestMapping("/admin/stock/allot/otherOut")
public class OtherOutController {
    ...
    @RequiresPermissions("/stock/allot/otherOut/index")
    @OperationLog(name = "index")
    @ApiOperation(value = "其他出库单列表", response = String.class)
    public ApiResult<Map<String, Object>> index(@RequestBody CommonPageParam param) {
        param.setBillType(BillTypeEnum.QTCKD.getValue());
        return ApiResult.ok(billHeadService.indexOrder(param));
    }

    //新增其他出库单
    @RequestMapping(value = "/add", method = RequestMethod.POST)
    @RequiresPermissions("/stock/allot/otherOut/add")
    @OperationLog(name = "add")
    @ApiOperation(value = "新增其他出库单", response = String.class)
    public ApiResult<String> add(@RequestBody @Validated BillHead head) {
        head.setBillType(BillTypeEnum.QTCKD.getValue());
    ...
    @RequiresPermissions("/stock/allot/otherOut/editDeliver")
