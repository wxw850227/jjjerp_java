<template>
  <div class="upload-wrap">
    <el-dialog title="系统图片管理" v-model="dialogVisible" :close-on-click-modal="false" custom-class="upload-dialog"
      ...
      <div class="fileContainer">
        <div class="file-type">
          <ul>
            <li :class="activeType==item.categoryId?'item active':'item'" v-for="(item,index) in typeList" :key="index"
              @click="selectTypeFunc(item.categoryId)">
              {{item.name}}
              <div class="operation" @click.stop v-if="item.categoryId!=null">
                <p @click="editCategoryFunc(item)"> <el-icon><Edit /></el-icon></p>
                <p @click="deleteCategoryFunc(item.categoryId)">  <el-icon><Delete /></el-icon></p>
              </div>
            </li>
          </ul>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getFileCategory() {
        ...
        FileApi.SystemPictureList(param, true).then(res => {
            self.loading = false;
