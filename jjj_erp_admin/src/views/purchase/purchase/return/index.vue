	...
	<div class="product bgset set-main">
		...
		<div class="common-level-rail d-b-c">
			<div class="d-s-c flex-1">
				<el-button size="small" type="primary" icon="Plus" @click="addClick"
					v-auth="'/product/base/unit/add'">直接出库</el-button>
				<el-button size="small" type="primary" icon="Plus" @click="procureAddClick"
					v-auth="'/product/base/unit/delete'">采购退货出库</el-button>
		...
		<el-table class="flex-1" size="small" :data="tableData" border stripe style="width: 100%"
							...
							<template #dropdown>
								<el-dropdown-menu>
									<el-dropdown-item @click="editClick(scope.row)"
										v-if="scope.row.auditStatus != 20">修改</el-dropdown-item>
			...
			<el-table-column prop="businessType" label="业务类型" width="110">
				<template #default="scope">
					<el-text effect="plain" type="danger" size="small" v-if="scope.row.businessType == 3">直接出库</el-text>
					<el-text effect="plain" type="warning" size="small"
						v-if="scope.row.businessType == 4">采购入库单退货</el-text>
				</template>
			</el-table-column>
			<el-table-column prop="codeNum" label="单据编号" width="100" show-overflow-tooltip></el-table-column>
			...
			<el-table-column width="120" prop="linkBillNum" label="采购订单" show-overflow-tooltip></el-table-column>
	...
	export default {
		...
		methods: {
			...
			getData(param = "") {
				...
				PurchaseApi.returnList(Params, true)
					.then((res) => {
						self.loading = false;
			...
			authClick(row) {
				let self = this;
				// 弹出确认框，供用户选择同意或拒绝
				ElMessageBox.confirm("确定要审核该订单吗？", "审核退货单", {
