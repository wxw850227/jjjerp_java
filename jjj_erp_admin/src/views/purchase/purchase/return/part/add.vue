<template>
	<el-dialog title="采购出库" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false"
		:close-on-press-escape="false" :fullscreen="fullscreen" :show-close="false" align-center>
		<template #header="{ close, titleId, titleClass }">
			<div class="my-header d-b-c">
				<div :id="titleId" :class="titleClass" class="flex-1">采购出库</div>
		...
		<el-table size="small" :data="productData" border style="width: 100%" @selection-change="handleSelectionChange"
			...
			<el-table-column label="操作" width="60">
				<template #default="scope">
					<el-button @click="editClick(scope.row)" type="primary" link size="small">关闭</el-button>
			...
			<el-table-column prop="imageUrl" label="图" width="42">
				<template #default="scope">
					...
					<el-image v-else style="width: 26px; height: 26px" :src="scope.row.imageUrl"
			...
			<el-table-column prop="purchaseDecimal" label="单价" width="100"><template #default="{ row }">
					<el-input v-if="row.productExtend" v-model="row.productExtend.purchaseDecimal" label="单价"
						@blur="handleUnitPriceChange(row)" />
					<el-input v-else v-model="row.purchaseDecimal" label="单价" @blur="handleUnitPriceChange(row)" />
				</template>
			</el-table-column>
		...
		<el-form size="small" :inline="true" :model="searchForm" :rules="formRules" ref="form" class="demo-form-inline dialogo-form-item">
			<el-row>
				<el-col :span="6">
					<el-form-item prop="discountType" label="优惠类型">
						<el-select v-model="searchForm.discountType" placeholder="请选择">
					...
					<el-form-item prop="discountMoney" label="优惠金额">
						<el-input v-model="searchForm.discountMoney" @blur="discountMoneyChange(searchForm)"
							placeholder="请输入优惠金额"></el-input>
					</el-form-item>
					</el-col>
					<el-col :span="6">
					<el-form-item prop="deposit" label="本次付款">
						<el-input v-model="searchForm.deposit" @blur="discountMoneyChange(searchForm)"
							placeholder="请输入本次付款"></el-input>
					</el-form-item>
					</el-col>
					<el-col :span="6">
					<el-form-item prop="deptMoney" label="欠款金额">
					...
					<el-row>
				     <el-col :span="6">
					<el-form-item prop="purchaseMoney" label="采购运费">
						<el-input v-model="searchForm.purchaseMoney"></el-input>
					</el-form-item>
					</el-col>
					<el-col :span="6">
					<el-form-item prop="payType" label="结算方式">
	...
	export default {
		...
		created() {
			...
			this.productData = this.allData.selectedData.filter(
				(item) => !item.billItemList
			);
			this.codeData = this.allData.selectedData.filter(
				(item) => item.billItemList
			);
			let codeNum1 = this.codeData.map((item) => item.codeNum);
			this.codeNum1 = JSON.stringify(codeNum1).replace(
				/[^\u4e00-\u9fa5a-zA-Z0-9]/g,
				""
			);
			// this.productData.splice(0, 0, this.codeNum1);
			...
			this.codeNums = this.codeData.map((item) => item.codeNum);
			console.log(this.codeNums, "this.codeNums");
		...
		methods: {
			...
			editUser() {
				...
				let billItemList = self.productData.map((option) => ({
					...
					unitPrice: option.productExtend ?
				...
				self.loading = true;
				PurchaseApi.returnAdd(params, true)
					.then((data) => {
						self.loading = false;
						ElMessage({
							message: "添加成功",
