<template>
	<el-dialog title="入库单" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false"
		:close-on-press-escape="false" :fullscreen="fullscreen" :show-close="false" align-center
		append-to="#right-content-box">
		<template #header="{ close, titleId, titleClass }">
			<div class="my-header d-b-c">
				<div :id="titleId" :class="titleClass" class="flex-1">入库单</div>
				...
				<el-row>
					...
					<el-col :span="5">
						<el-form-item label="供应商">
							<el-select v-model="searchForm.supplierId" :disabled="openDetail" placeholder="请选择">
								<el-option v-for="(item, index) in supplierList" :key="index" :label="item.supplierName"
									:value="item.supplierId" />
							</el-select> </el-form-item></el-col>
					<el-col :span="5">
						<el-form-item label="仓库">
							<el-select v-model="searchForm.depotId" :disabled="openDetail" placeholder="请选择">
								<el-option v-for="(item, index) in depotList" :key="index" :label="item.name"
									:value="item.depotId" />
							</el-select> </el-form-item></el-col>
					<el-col :span="5"><el-form-item label="单据编号">
							<el-input size="small" :disabled="openDetail" v-model="searchForm.codeNum"
				...
				<el-row>		
					...
					<el-col :span="5">
						<el-form-item label="采购订单">
							<el-input size="small" :disabled="openDetail" v-model="searchForm.linkBillNum"
								placeholder=""></el-input> </el-form-item>
								</el-col>
					<el-col :span="14">
						<el-form-item label="备注" class="ww100">
							<el-input size="small" :disabled="openDetail" v-model="searchForm.remark"
								placeholder=""></el-input> </el-form-item></el-col>
				</el-row>
				<el-row>
					<el-col :span="3">
						<el-form-item>
							<el-button size="small" type="primary">导入商品明细</el-button>
						</el-form-item></el-col>
					<el-col :span="10">
						<el-form-item label="快速选择商品:" class="ww100">
							<el-input :disabled="openDetail" size="small" @keyup.enter="reserch()"
								v-model="searchForm.productCode" placeholder="输入编码/名称(精确到规格)后按回车">
								<template #append>
									<el-button @click="reserch" :disabled="openDetail" icon="Search" />
								</template>
		...
		<el-table size="small" :data="productData" border style="width: 100%" @selection-change="handleSelectionChange"
			...
			<el-table-column prop="productExtendId" label="ID" width="55"></el-table-column>
			...
			<el-table-column prop="imageUrl" label="图" width="42">
				<template #default="scope">
					...
					<el-image v-if="scope.row.imageUrl" style="width: 26px; height: 26px" :src="scope.row.imageUrl"
			...
			<el-table-column prop="productExtend.name" label="商品名称" width="80" show-overflow-tooltip>
				<template #default="scope">
					<span v-if="scope.row.productExtend">
			...
			<el-table-column prop="availableAllNumber" label="可入库总数" width="100"></el-table-column>
			<el-table-column prop="operNumber" label="实收数量" width="100">
				<template #default="{ row }">
			...
			<el-table-column prop="basicNumber" label="总数(pcs)" width="100">
				<template #default="{ row }">
					<el-input v-model="row.basicNumber" :disabled="openDetail" label="数量" />
				</template>
			</el-table-column>
			...
			<el-table-column prop="allPrice" label="总价" width="100"><template #default="{ row }">
					<el-input v-model="row.allPrice" :disabled="openDetail" disabled />
				</template>
			</el-table-column>
			<el-table-column prop="discountRate" label="折扣率" width="120"><template #default="{ row }">
					<el-input v-model="row.discountRate" @blur="handleDiscountRateChange(row)" :disabled="openDetail"
			...
			<el-table-column prop="discountPrice" label="折后单价" width="100"><template #default="{ row }">
					<el-input v-model="row.discountPrice" :disabled="openDetail" disabled />
				</template>
			</el-table-column>
			...
			<el-table-column prop="remark" label="备注">
				<template #default="{ row }">
					<el-input v-model="row.remark" :disabled="openDetail" label="备注" />
				</template>
			</el-table-column>
		</el-table>
			...
			<el-row>
				<el-col :span="6">
					<el-form-item label="优惠类型">
						<el-select v-model="searchForm.discountType" :disabled="openDetail" placeholder="请选择">
		...
		<template #footer>
			<div class="dialog-footer">
				...
				<el-button type="primary" v-if="!openDetail" @click="editUser" :loading="loading">确 定</el-button>
			</div>
		</template>
	...
	export default {
		...
		methods: {
			...
			closeClick(row) {
				...
				if (index !== -1) {
					// 修改该行的 isdelete 值为 1
					this.productData[index].isDelete = 1;
					// 移除该行
			...
			editUser() {
				...
				params.totalPrice = this.totalPrices;
				params.billItemList = billItemList;
				console.log(params, "params");
				self.loading = true;
				PurchaseApi.storageEdit(params, true)
					.then((data) => {
						self.loading = false;
						ElMessage({
							message: "修改成功",
