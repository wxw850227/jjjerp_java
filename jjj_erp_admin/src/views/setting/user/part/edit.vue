        ...
        <el-row>
          <el-col :span="8">
            ...
            >
              ...
              >
                <el-option
                  v-for="(item, index) in platformType"
                  :key="index"
                  :label="item.name"
                  :value="item.value"
                />
          ...
          >
            ...
            <el-table-column prop="city" label="市">
              <template #default="scope">
                ...
                >
                  <el-option
                    v-for="city in scope.row.province &&
                    areaData.find((item) => item.name === scope.row.province)
                      .children"
                    :key="city.id"
                    :label="city.name"
                    :value="city.name"
            ...
            <el-table-column prop="region" label="区">
              <template #default="scope">
                ...
                >
                  <el-option
                    v-for="region in scope.row.city &&
                    areaData
                      .find((item) => item.name === scope.row.province)
                      .children.find((item) => item.name === scope.row.city)
                      .children"
                    :key="region.id"
                    :label="region.name"
                    :value="region.name"
...
export default {
  ...
  methods: {
    ...
    getdata() {
      ...
      SettingApi.userToEdit({ userId: userId }).then((res) => {
    ...
    addUser() {
      ...
      params.addressList = this.addressList;
      console.log(params, "params");
      self.$refs.form.validate((valid) => {
        if (valid) {
          self.loading = true;
          SettingApi.userEdit(params)
            .then((data) => {
              self.loading = false;
              ElMessage({
                message: "修改成功",
