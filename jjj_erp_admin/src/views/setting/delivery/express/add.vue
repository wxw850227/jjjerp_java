<template>
	<el-dialog title="新增物流公司" v-model="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false"
		...
		<template #header="{ close, titleId, titleClass }">
			<div class="my-header d-b-c">
				<div :id="titleId" :class="titleClass" class="flex-1">新增物流公司</div>
		...
		<el-form ref="ruleFormRef" style="max-width: 600px" :model="ruleForm" :rules="rules" label-width="auto"
			...
			<el-form-item label="物流公司名称 " prop="expressName" :rules="[{ required: true, message: ' ' }]">
				<el-input v-model="form.expressName" autocomplete="off"></el-input>
				<div class="tips">
					请对照
		...
		methods: {
			...
			onSubmit() {
				...
				self.$refs.form.validate((valid) => {
					if (valid) {
						self.loading = true;
						SettingApi.expressAdd(form, true)
							.then((data) => {
								self.loading = false;
								ElMessage({
									message: "恭喜你，添加成功",
